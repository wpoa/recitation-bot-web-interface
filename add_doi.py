...
print "<input type='checkbox' name='reupload' />Force reupload if doi already uploaded<BR/>"
print "<input type='submit' value='submit form' />"
print "</form>"
form = cgi.FieldStorage()

reupload = 'reupload_off'
#print form
for field_name in form:
    field=form[field_name]
    if field.name == 'reupload':
        if form['reupload'].value == 'on':
            print "<p>reupload: %s</p>" % form['reupload'].value
            reupload = 'reupload_on'
    if field.name == 'doi':
        ...
        if form['doi'].value:
            ...
            dequeue.write(doi_plain+'\t'+reupload+'\n')
            dequeue.close()
